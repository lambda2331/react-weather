...
import { errorCallback, successCallback } from './callback'
import { WeatherInfo } from '../types/common'

const apiKey: string = process.env.REACT_APP_OPEN_WEATHER_MAP_API_KEY as string
...
const generateURLQuery = (params: WeatherURLQueryParams): string => {
  const queryData = Object.entries(params).reduce((settings, [key, value]: [string, string]) => {
...
export const weatherApi = {
  getWeatherByCity (city: string): Promise<WeatherInfo> {
    ...
    return httpClient.get<WeatherInfo>(`weather?${urlQuery}`, config)
      .then(successCallback, () => errorCallback(Error(city)))
  }
}
